...
import { recipientStore } from '@/entities';

import styles from './styles.module.scss';

const RecipientsList = () => {
    const recipients = recipientStore.use.recipients();

    const preview = useStateCustom(null);
    const activeIndex = useStateCustom(-1);

    const showPreview = (index: number) => {
        preview.set(recipients.value[index]);
        activeIndex.set(index);
    };

    const remove = () => {
        recipients.set(recipients.value.filter((i, index) => index !== activeIndex.value));
        preview.set(null);
        activeIndex.set(-1);
    };
    ...
    return (
        <div className={styles.wrapper}>
            <div className={styles.title}>Список получателей</div>
            <div className={styles.container}>
                <div className={styles.list}>
                    {recipients.value.map((config, index) => (
                        <div
                            key={index}
